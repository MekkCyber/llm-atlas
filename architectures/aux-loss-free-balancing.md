...
$$
s_{i,t} = \mathrm{sigmoid}( h_t \cdot e_i ) \quad \text{(affinity score, unchanged)}
$$

$$
\tilde{s}_{i,t} = s_{i,t} + b_i \quad \text{(biased score — used ONLY to pick which experts to route to)}
$$

$$
S_t = \{ \text{top-}K_r \text{ experts by } \tilde{s}_{i,t} \}
$$

$$
g_{i,t} = \frac{s_{i,t}}{\sum_{j \in S_t} s_{j,t}} \quad \text{for } i \in S_t \quad \text{(gating weights — use the UNBIASED score, normalized)}
$$
...
After each step, count how many tokens were routed to each expert ($n_i$) and compare to the ideal balanced count $\bar{n} = B \cdot K_r / N$ (where $B$ is batch size). Update biases:
...
$\gamma = 0.001$ in DeepSeek-V3, for the first 14.3T tokens of training; then $\gamma = 0.0$ for the final 500B (so the final model has frozen biases).
...
- **Tuning-free.** $\gamma = 0.001$ works across model sizes. No coefficient to sweep, no schedule to design.
...
- **Update $\gamma$ scheduling.** Too large and the bias oscillates (an expert flips from overloaded to underloaded every step). Too small and it can't track rapid router shifts at the start of training. $\gamma = 0.001$ is empirical; don't go above 0.01.
- **Freeze the bias near the end of training.** DeepSeek-V3 drops $\gamma$ to 0 for the last 500B tokens so the final checkpoint has fixed biases. Otherwise, last-batch stochasticity leaves the final model with a slightly perturbed bias vector.
...
- **Count the right thing.** The bias update uses $n_i$ = number of tokens routed to expert $i$ in this step. Using gate-weighted counts (e.g. $\sum_t g_{i,t}$) or probability masses ($\sum_t s_{i,t}$) gives subtly different control dynamics — stick with raw token counts.
- **Batch size matters.** Smaller batches have noisier $n_i$, so the control loop has more jitter. At very small batch sizes ($B \cdot K_r < N$, so on average not every expert even gets a token) the bias update can go unstable — consider accumulating counts over multiple steps before updating.
...
- Paper: *DeepSeek-V3 Technical Report* — DeepSeek, 2024 — large-scale application with $\gamma = 0.001$, paired with a small sequence-wise guardrail.
