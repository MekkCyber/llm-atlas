...
1. Project into $h$ separate $(Q, K, V)$ triples, each of dimension $d_k = d_{\text{model}} / h$.
2. Run scaled dot-product attention independently on each head.
3. Concatenate the $h$ outputs back to $d_{\text{model}}$.
...
- Project $X \to QKV$ of shape $(n, 3 \cdot d_{\text{model}})$.
- Reshape to $(h, n, d_k)$ per $Q, K, V$.
- Run batched attention across the head dimension.
- Reshape back to $(n, d_{\text{model}})$.
- Multiply by $W_O$.
...
- **Multi-Query Attention (MQA)** — one K and one V head shared across all Q heads. Cuts KV cache size by $h\times$, which dominates memory at inference. Quality cost is noticeable at scale.
...
- **Head count vs. head dim tradeoff.** More heads with smaller $d_k$ = more specialization but each head has less capacity. The typical choice is $d_k = 64$ or $128$; head count follows from $d_{\text{model}} / d_k$.
