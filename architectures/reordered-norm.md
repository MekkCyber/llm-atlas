...
$$
x_1 = x + \mathrm{Norm}(\mathrm{Attention}(x))
$$

$$
x_2 = x_1 + \mathrm{Norm}(\mathrm{FFN}(x_1))
$$
...
- **Initialization.** RMSNorm $\gamma = 1.0$ works. No special init of residual branch scaling required (in contrast with post-norm where DeepNet-style scaling is sometimes needed).
...
- **Interaction with residual scaling.** If you additionally scale residual branches (e.g., dividing by $\sqrt{\text{depth}}$), measure the effect — the combination can over-suppress sub-layer contribution.
