...
Sequence-wise balance loss tightens the scope: the sum $\sum_t$ in $f_i$ and $P_i$ is taken over tokens of *one sequence* only. A separate balance penalty is computed for every sequence. Balance is enforced everywhere, not just on average.
...
$$
f_i = \frac{N_r}{K_r \cdot T} \sum_{t=1}^{T} \mathbf{1}\{ s_{i,t} \in \mathrm{Top}\text{-}K_r(\{s_{j,t}\}_{j=1}^{N_r}) \}
$$
...
$$
\mathrm{loss\_seq} = \alpha \cdot \sum_{i=1}^{N_r} f_i \cdot P_i
$$

The normalization constants make $f_i = 1/N_r$ and $P_i = 1/N_r$ under uniform routing, so $\sum_i f_i \cdot P_i = 1/N_r$ and the loss is $\alpha / N_r$ at perfect balance.
...
$$
L_{\text{seq\_total}} = \sum_{\ell \in \text{MoE layers}} \sum_{s \in \text{sequences in batch}} \mathrm{loss\_seq}^{(\ell, s)}
$$
...
| Workable coefficient $\alpha$ | ~$10^{-2}$ (Switch) | ~$10^{-3}$–$10^{-4}$ (DeepSeekMoE / V3) |
...
- **Cheap safety net on top of aux-loss-free.** At $\alpha = 10^{-4}$ the gradient interference is below noise; you get the single-sequence protection essentially for free.
...
- **Stricter balance ⇒ less specialization.** A sequence of pure code tokens, under sequence-wise balance, must spread its routing across all experts — even though sending code tokens to non-code experts is semantically wrong. Large $\alpha$ in a sequence-wise formulation can hurt domain specialization.
- **High variance of the statistic.** A 4K-token sequence computing $f_i$ over 256 experts has on average $4000 \cdot 8 / 256 \approx 125$ tokens per expert. This is a noisy estimate. Very large $\alpha$ amplifies the noise into the router gradient.
- **Don't reuse a batch-wise $\alpha$.** Switch's $\alpha = 10^{-2}$ is for a batch statistic with $T$ in the millions. Applied to sequence-scope, the same coefficient is roughly 2–3 orders of magnitude too strong for the signal quality.
- **The $N_r / (K_r \cdot T)$ normalization is non-obvious.** It's chosen so that under perfectly uniform top-$K_r$ routing, $f_i = 1/N_r$ exactly — match-up with $P_i$'s $1/N_r$ under uniform sigmoid scores, so the product-and-sum lands on $1/N_r$ at optimum.
...
- Paper: *DeepSeek-V3 Technical Report* — DeepSeek, 2024 — Sec 2.1.2, Eqs. 17–20 and Sec 4.2. Introduces the label "sequence-wise", adds the sigmoid-normalization step, reduces $\alpha$ to $10^{-4}$ as a safety net on top of aux-loss-free balancing.
