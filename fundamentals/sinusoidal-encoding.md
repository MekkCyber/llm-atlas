...
**Related:** [_positional-encoding](_positional-encoding.md) · [rope](rope.md)

---

## What it is

A function $\mathrm{PE} : \mathbb{N} \to \mathbb{R}^d$ that maps a position index to a $d$-dimensional vector, using pairs of (sin, cos) at different frequencies:
...
- $\mathrm{pos}$ is the token's position in the sequence ($0, 1, 2, \ldots$).
- $i$ indexes into the embedding dimension ($0 \ldots d/2 - 1$).
...
The key property: for any fixed offset $k$, the encoding $\mathrm{PE}(\mathrm{pos} + k)$ can be written as a **linear function of $\mathrm{PE}(\mathrm{pos})$**. Concretely, because
...
the shift from $\mathrm{pos}$ to $\mathrm{pos} + k$ is a rotation in each $(\sin, \cos)$ pair by a fixed angle. Attention can, in principle, learn a linear projection that turns this rotation into a relative-position signal. So although the encoding itself is absolute, relative offsets are linearly recoverable from it.

### Where it's injected

$$
x_0 = \mathrm{TokenEmbedding}(\text{tokens}) + \mathrm{PE}(\text{positions})
$$
...
- **Not invariant to scale.** Scaling up $d_{\text{model}}$ without adjusting the base wavelength changes the effective frequency spectrum.
