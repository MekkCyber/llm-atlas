...
$$
L_{\text{ce}} = -\log \mathrm{softmax}(\text{logits})[\text{target}] = -\text{logits}[\text{target}] + \log Z
$$

$$
\text{with } Z = \sum_v \exp(\text{logits}[v])
$$

$\log Z$ is the log-partition function — a scalar summary of "how large the logits are overall". Cross-entropy cares only about the *difference* $\text{logits}[\text{target}] - \log Z$, so it doesn't pin down the absolute magnitude of the logits. Nothing stops the logits (and $\log Z$ with them) from drifting upward indefinitely.

Z-loss adds a soft constraint that $\log Z$ stays near zero:

$$
L = L_{\text{ce}} + \alpha \cdot (\log Z)^2
$$

Typical $\alpha \approx 10^{-4}$. Applied only to the final output logits, not intermediate projections.
...
1. **Gradient collapse.** When $\exp(\text{logit}_{\max}) \gg \sum_{v \ne \max} \exp(\text{logit}_v)$, the softmax output is $(\approx 1, 0, \ldots, 0)$. The gradient of $\mathrm{softmax}(z)_i$ with respect to $z_j$ is $\mathrm{softmax}(z)_i (\delta_{ij} - \mathrm{softmax}(z)_j)$. At saturation this is $\approx 0$ everywhere except the argmax entry. The LM head stops receiving useful signal.
2. **Numerical instability.** $\log Z$ drifting into the hundreds means $\exp(\text{logit}_{\max})$ overflows in fp16/bf16. Mixed-precision training blows up with NaN.

### What z-loss does

The gradient of $\alpha \cdot (\log Z)^2$ with respect to the logits is

$$
\frac{\partial}{\partial \text{logits}[v]} \alpha (\log Z)^2 = 2\alpha \cdot \log Z \cdot \mathrm{softmax}(\text{logits})[v]
$$
...
- **No hyperparameter pain.** $\alpha = 10^{-4}$ works across model sizes. Not a tuning knob.
...
- **Use logsumexp.** Computing $\log \sum \exp(\text{logits})$ directly overflows; `logsumexp` is numerically stable.
