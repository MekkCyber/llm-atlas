...
**TL;DR:** Start from the KL-regularized RLHF objective. Under Bradley-Terry pairwise preferences, there's a **closed-form relationship** between the optimal policy and the latent reward: $\pi^*(y \mid x) \propto \pi_{\text{ref}}(y \mid x) \cdot \exp(r(x, y) / \beta)$. Rearrange to write the reward as a log-ratio of policies, plug into the BT preference log-likelihood, and the reward model vanishes — you can train directly on preference pairs via a simple binary cross-entropy. **No reward model, no rollouts, no PPO, no value network.** Just one offline pass over $(\text{prompt}, \text{preferred}, \text{rejected})$ triples. Used by Zephyr, Tülu 3, Llama 3 alignment, and most modern open-recipe preference tuning.
...
2. **Reward model** $r_\phi(x, y)$ trained on pairwise preferences via Bradley-Terry MLE.
3. **RL** — PPO against $r_\phi$ with a KL penalty to the SFT checkpoint.
...
Given:
- $\pi_\theta$ = trainable policy, $\pi_{\text{ref}}$ = reference (usually the SFT checkpoint, frozen).
- Preference data $\mathcal{D} = \{(x_i, y_w, y_l)\}$ — preferred completion $y_w$, dispreferred $y_l$.
...
Classical RLHF fits $r_\phi$ to the preferences (BT MLE), then optimizes:

$$
\max_{\pi_\theta}\; \mathbb{E}_{x \sim \mathcal{D},\, y \sim \pi_\theta} [\, r_\phi(x, y) \,] - \beta \cdot \mathrm{KL}( \pi_\theta(\,\cdot\, \mid x) \,\|\, \pi_{\text{ref}}(\,\cdot\, \mid x) )
$$

$\beta > 0$ is the KL regularization strength.
...
$$
\pi_r(y \mid x) = \frac{1}{Z(x)} \cdot \pi_{\text{ref}}(y \mid x) \cdot \exp\!\left( \frac{r(x, y)}{\beta} \right)
$$

$$
Z(x) = \sum_y \pi_{\text{ref}}(y \mid x) \cdot \exp\!\left( \frac{r(x, y)}{\beta} \right) \quad \text{(partition function)}
$$
...
$$
r(x, y) = \beta \cdot \log\!\left( \frac{\pi_r(y \mid x)}{\pi_{\text{ref}}(y \mid x)} \right) + \beta \cdot \log Z(x)
$$

Every $(\text{policy}, \pi_{\text{ref}})$ pair **induces a reward**, up to a prompt-only shift $\beta \cdot \log Z(x)$ (which doesn't depend on $y$).
...
The $\beta \cdot \log Z(x)$ shift cancels (same term for $y_w$ and $y_l$). Substituting the reward-as-log-ratio expression:

$$
p^*(y_w \succ y_l \mid x) = \sigma\!\left( \beta \cdot \log\frac{\pi^*(y_w \mid x)}{\pi_{\text{ref}}(y_w \mid x)} - \beta \cdot \log\frac{\pi^*(y_l \mid x)}{\pi_{\text{ref}}(y_l \mid x)} \right)
$$

### Step 4 — the DPO loss

Identify $\pi^*$ with the policy we're training, $\pi_\theta$. Take the negative log-likelihood over the preference dataset:

$$
\mathcal{L}_{\text{DPO}}(\pi_\theta;\, \pi_{\text{ref}}) = -\mathbb{E}_{(x, y_w, y_l) \sim \mathcal{D}} \!\left[ \log \sigma\!\left( \beta \cdot \log\frac{\pi_\theta(y_w \mid x)}{\pi_{\text{ref}}(y_w \mid x)} - \beta \cdot \log\frac{\pi_\theta(y_l \mid x)}{\pi_{\text{ref}}(y_l \mid x)} \right) \right] \quad \text{(Rafailov 2023, Eq. 7)}
$$

This is a **binary cross-entropy** loss — same shape as a classifier. The reward model is gone: every term is a log-probability under $\pi_\theta$ or $\pi_{\text{ref}}$, both of which you can evaluate in one forward pass.

### The gradient

Let $\hat{r}_\theta(x, y) = \beta \cdot \log( \pi_\theta(y \mid x) / \pi_{\text{ref}}(y \mid x) )$ be the **implicit reward**. Then:

$$
\nabla_\theta \mathcal{L}_{\text{DPO}} = -\beta \cdot \mathbb{E}_{(x, y_w, y_l) \sim \mathcal{D}} \!\left[ \sigma( \hat{r}_\theta(x, y_l) - \hat{r}_\theta(x, y_w) ) \cdot ( \nabla_\theta \log \pi_\theta(y_w \mid x) - \nabla_\theta \log \pi_\theta(y_l \mid x) ) \right]
$$

Three pieces:

- **$\nabla_\theta \log \pi_\theta(y_w \mid x)$** — pushes up the likelihood of the preferred completion.
- **$-\nabla_\theta \log \pi_\theta(y_l \mid x)$** — pushes down the likelihood of the rejected completion.
- **$\sigma(\hat{r}_\theta(x, y_l) - \hat{r}_\theta(x, y_w))$** — a per-sample weight that is *large* exactly when the implicit reward model **gets the pair wrong** (rates $y_l$ above $y_w$). On correctly-ordered pairs, the sigmoid is small and the update shrinks toward zero.

The weighting is what makes DPO different from naive unlikelihood training (maximize $\log \pi(y_w)$ and minimize $\log \pi(y_l)$ unweighted) — the paper reports that naive unlikelihood causes model degeneration.

### Ranked generalization (Plackett-Luce)

For datasets with full rankings $\tau$ over $K > 2$ candidates, DPO generalizes to a Plackett-Luce log-likelihood (Appendix A.3):

$$
\mathcal{L}_{\text{DPO}} = -\mathbb{E}_{\tau,\, y_{1..K},\, x \sim \mathcal{D}} \!\left[ \log \prod_{k=1}^{K} \frac{\exp(\beta \cdot \log(\pi_\theta(y_{\tau(k)} \mid x) / \pi_{\text{ref}}(y_{\tau(k)} \mid x)))}{\sum_{j=k}^{K} \exp(\beta \cdot \log(\pi_\theta(y_{\tau(j)} \mid x) / \pi_{\text{ref}}(y_{\tau(j)} \mid x)))} \right]
$$
...
- $\beta = 0.1$ is the default (Anthropic HH, sentiment).
- $\beta = 0.5$ for TL;DR summarization.
- Sentiment sweep used $\beta \in \{0.05, 0.1, 1, 5\}$ to trace the reward-vs-KL frontier.
- Batch size 64, RMSprop, LR $10^{-6}$ with linear warmup over 150 steps.
- $\beta$ **large** → stays close to $\pi_{\text{ref}}$ (strong KL). $\beta$ **small** → free to move far (weak KL). In the DPO loss, $\beta$ also sets the *margin* the sigmoid operates over — larger $\beta$ $\Rightarrow$ sharper per-example weighting.
...
- **The paper's theoretical bonus.** Section 5.2 shows that PPO's reward is missing the normalizer $\beta \cdot \log \sum_y \pi_{\text{ref}}(y \mid x) \exp(r(x, y) / \beta)$ — the *soft value function*. Classical RLHF works around this with human-completion baselines (Monte-Carlo estimates). DPO's reparameterization absorbs the normalizer automatically, so no baseline is needed.
...
- **$\pi_{\text{ref}}$ must match the preference distribution.** If $\pi_{\text{ref}} \ne \pi_{\text{SFT}}$, the implicit reward estimate is biased. The paper recommends $\pi_{\text{ref}} = \pi_{\text{SFT}}$. When no SFT is available (HH), they suggest fitting $\pi_{\text{ref}}$ by SFT on the *preferred* completions only — crude but works.
...
- **$\beta$ is a global knob, not a schedule.** Unlike PPO's adaptive KL, DPO's $\beta$ is fixed. Some follow-ups ($\beta$-scheduled DPO) anneal it; the original doesn't.
- **Naive unweighted unlikelihood loss degenerates.** Dropping the $\sigma(\hat{r}_\theta(x, y_l) - \hat{r}_\theta(x, y_w))$ weight and just maximizing $\log \pi(y_w) - \log \pi(y_l)$ unweighted causes mode collapse and nonsensical output. The dynamic weighting is load-bearing.
- **Implicit reward $\hat{r}_\theta$ is interpretable.** $\hat{r}_\theta(x, y) = \beta \cdot \log(\pi_\theta(y \mid x) / \pi_{\text{ref}}(y \mid x))$ can be logged during training to monitor convergence — rising $\hat{r}(y_w) - \hat{r}(y_l)$ margin means the policy is learning.
- **No guarantee $y_w$ gets absolutely more probable.** DPO can push $\log \pi(y_w)$ *down* as long as $\log \pi(y_l)$ goes down faster. Watch the absolute log-prob of preferred completions, not just the margin. Some deployments see both go down — the model is optimizing the ranking, not the likelihood.
...
- **Reward over-optimization exists differently in DPO.** Classical RLHF over-optimization happens against a trained RM. DPO's equivalent is over-optimizing against the implicit reward implied by $\pi_{\text{ref}}$ — if the preference set is inconsistent with $\pi_{\text{ref}}$'s distribution, the policy can over-fit the preference data at the cost of general capability. The paper's Fig. 3-right shows a slight drop at high training; follow-ups study this as a DPO-specific phenomenon.
