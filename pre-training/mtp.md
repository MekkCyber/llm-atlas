...
$$
L_{\text{main}} = -\sum_t \log P(x_{t+1} \mid x_{\le t})
$$

MTP adds additional prediction heads that, at position $t$, predict tokens $t+2, t+3, \ldots, t+k$ — each through its own small transformer module sitting on top of the main backbone. The extra losses are summed with a coefficient into the training objective; the extra modules can be discarded at inference for the base model, or kept for **speculative decoding**.
...
Inputs: the previous step's hidden state $h_t^{\text{MTP},k-1}$ (or $h_t^{\text{main}}$ for the first module) and the embedding of $x_{t+k}$. These are combined via RMSNorms and a learned linear map, then passed through a transformer block:

$$
h'_t = W_m \cdot [\, \mathrm{RMSNorm}(h_t^{\text{MTP},k-1}) ;\, \mathrm{RMSNorm}(\mathrm{Embed}(x_{t+k})) \,]
$$

$$
h_t^{\text{MTP},k} = \mathrm{TransformerBlock}(h'_t)
$$

$$
P(x_{t+k+1} \mid \ldots) = \mathrm{softmax}( W_{\text{head}} \cdot h_t^{\text{MTP},k} )
$$

Crucially, **the embedding layer $\mathrm{Embed}$ and the output head $W_{\text{head}}$ are shared with the main model.** Only the fusion matrix $W_m$ and the transformer block's parameters are MTP-specific. This keeps the parameter overhead small.

### The loss

Sum the main and MTP losses with a coefficient $\lambda$:

$$
L = L_{\text{main}} + \lambda \cdot \frac{1}{D} \sum_{k=1}^{D} L_{\text{MTP},k}
$$

In DeepSeek-V3:
- $\lambda = 0.3$ for the first **10T tokens** of training
- $\lambda = 0.1$ for the final **4.8T tokens**
...
To predict $x_{t+2}$ from position $t$, the MTP module takes $h_t^{\text{main}}$ *plus* $\mathrm{Embed}(x_{t+2-1}) = \mathrm{Embed}(x_{t+1})$. The $x_{t+1}$ is not "future information" — at training time the entire sequence is known, and the module is only asked to predict the *next* token in its own stream. Just make sure the causal mask applies correctly so the main backbone at position $t$ never sees $x_{>t}$.
...
- **Natural curriculum.** Early training benefits more from dense supervision ($\lambda = 0.3$), late training from focus on the real objective ($\lambda = 0.1$). The schedule matters.
...
- **Fusion matrix size.** The $W_m$ that combines $h_t^{\text{MTP},k-1}$ and $\mathrm{Embed}(x_{t+k})$ needs to project $2 \cdot d_{\text{model}}$ down to $d_{\text{model}}$. Keep it simple — a single linear, no activation.
...
- **Decay $\lambda$ late in training.** The final 4.8T tokens with $\lambda = 0.1$ matters — dropping $\lambda$ to 0 too early loses the representation regularization; keeping it high to the end leaves the main objective under-optimized.
...
- Paper: *DeepSeek-V3 Technical Report* — DeepSeek, 2024 — depth-1 MTP at 671B scale with the $\lambda$ schedule and speculative-decoding integration.
